...
shared.LoaderTitle = "       Verifying Identification Please Wait....";
...
(getgenv()).Load = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("AstroHub") then
			makefolder("AstroHub");
		end;
		if not isfolder("AstroHub/Blox Fruits/") then
			makefolder("AstroHub/Blox Fruits/");
		end;
		if not isfile(("AstroHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("AstroHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(_G.Settings));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("AstroHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json"));
...
(getgenv()).SaveSetting = function()
	if readfile and writefile and isfile and isfolder then
		if not isfile(("AstroHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
			...
			writefile("AstroHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
		end;
	else
		return warn("Status : Undetected Executor");
	end;
...
PlayerTweenSpeedSlider = Tabs.SettingsTab:Slider({
	...
	print("Player Tween Speed: " .. value)
	end
});
...
spawn(function()
	while wait() do
		...
		elseif _G.Settings.Setting["Fast Attack Mode"] == "Ultimate Fast" then
		    _G.Settings.Setting["Fast Attack Delay"] = 0.5;
		end;
	end;
end);
spawn(function()
...
RainFruitButton = Tabs.FruitTab:Button({
	...
	WindUI:Notify({
         Title = "Notification",
         Content = "It's Just Visuals Not Real!",
         Icon = "network-device",
         Duration = 3
    });
		rainFruit();
